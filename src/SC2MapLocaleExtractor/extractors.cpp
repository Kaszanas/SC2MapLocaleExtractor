...
std::optional<std::string> extract_map_name_string(std::string game_description)
{
    ...
    std::vector<std::string> newline_split = split_string(game_description, "\r\n");
     ...
     if(it != newline_split.end()) {
         ...
         std::vector<std::string> map_name_line_split = split_string(parsed_line, "=");
...
std::optional<nlohmann::json> locale_extraction_pipeline(const std::string& filepath)
{
    ...
    directory_reader(detected_files, filepath, ".SC2Map");
    ...
    std::optional<nlohmann::json> maybe_final_mapping = generate_final_locale_mapping(mapping_container);
